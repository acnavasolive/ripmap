...
<img src="https://github.com/acnavasolive/ripmap/blob/main/figures/ripmap-logo.png" width="250">
...
The intuition behind the visual inspection relies not only on the amplitude of the ripple cycles exceeding some threshold (objectively trivial), but in other less quantifiable factors. (1) The size of ripple cycles have to change in a progressive fashion, from low to high amplitude and to low amplitude back (with some exception we cannot explain yet). (2) The frequency of those cycles has to be consistent, and although cycle length is known to vary throughout the ripple, the variation has to be progressive too (again, there are also some exceptions that we cannot explain yet). (3) How these trends compare to the surrounding LFP visually allows to determine what cycles belong to a ripple and which not. When there is a sudden change from non-progressive LFP oscillations to LFP oscillations in the ripple band with smooth changes, we can visually identify the beginning and end of the ripple, and that can be used to determine if there is a HFO in the window.  
