function [ripples] = findRipples_a(varargin)
...
addParameter(p,'show','off',@isstr) %changed by Anna, default was off
...
addParameter(p,'computeEMG',true,@islogical); % by default true 
...
elseif isnumeric(varargin{1}) && length(varargin{1})>2 % if first arg is LFP
...
if ~isempty(noise)
	...
	excluded = false(size(ripples,1),1);
...
if EMGThresh
    ...
    excluded = false(size(ripples,1),1);
    for i = 1:size(ripples,1)
       [~, ts] = min(abs(ripples(i,1)-EMGFromLFP.timestamps)); % get closest sample
...
ripples.lfp = lfp;
ripples.signal = signal; % low-pass signal
